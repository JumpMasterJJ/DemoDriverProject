...
WAIT_LIST_ENTRY*
FindWaitEntryByID(PLIST_ENTRY lpListHeader, ULONG ulWaitID)
{
    PLIST_ENTRY			lpList		= NULL;
    WAIT_LIST_ENTRY		*lpEntry	= NULL;
    ...
    for (lpList = lpListHeader->Flink; lpList != lpListHeader; lpList = lpList->Flink)
    {
        ...
        if (lpEntry->m_ulWaitID == ulWaitID)
        {
            return lpEntry;
        }
    }
...
BOOLEAN
CompletePendingIrp(LIST_ENTRY* IrpListHead, OP_INFO* lpOpInfo)
{
	...
	if (IsListEmpty(IrpListHead) == TRUE)
	{
		return bRet;
	}
	
	for (pIrpList = IrpListHead->Flink; pIrpList != IrpListHead; pIrpList = pIrpList->Flink)
	{
		lpIrp = CONTAINING_RECORD(pIrpList, IRP, Tail.Overlay.ListEntry);
	...
	RtlCopyMemory(lpIrp->AssociatedIrp.SystemBuffer, lpOpInfo, sizeof(RING3_OP_INFO));
